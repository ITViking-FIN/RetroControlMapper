...
async function loadSystems() {
  ...
  // v0.1.6: server-reported client version, used by buildCommunitySubmitUrl
  // so the GitHub Issue form's `client_version` field pre-fills correctly.
  if (data.version) window.RBCF_VERSION = data.version;
  selSystem.innerHTML = '';
...
function buildCommunitySubmitUrl(system, rom, bindings) {
  // v0.1.6: align with .github/ISSUE_TEMPLATE/community-binding.yml.
  // The form's `template=` param uses the YAML's per-field IDs (system,
  // game, bindings, client_version) to pre-fill each input. Falling back
  // to a plain title+body issue if the template isn't present on the repo.
  const owner = (window.RBCF_GH_OWNER || 'ITViking-FIN');
  const repo  = (window.RBCF_GH_REPO  || 'RetroControlMapper');
  const clientVersion = (window.RBCF_VERSION || '0.1.6');
  const title = `[bindings] ${system}: ${rom}`;
  const bindingsJson = JSON.stringify(
    { system_id: system, rom_name: rom, bindings }, null, 2);
  const url = `https://github.com/${owner}/${repo}/issues/new`
            + `?template=community-binding.yml`
            + `&title=${encodeURIComponent(title)}`
            + `&system=${encodeURIComponent(system)}`
            + `&game=${encodeURIComponent(rom)}`
            + `&bindings=${encodeURIComponent(bindingsJson)}`
